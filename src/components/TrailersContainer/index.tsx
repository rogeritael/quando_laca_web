        ...
        <div style={{ width: "100%" }}>
            <TrailersContainerSection>
                ...
                <div className="desktop">
                    {trailers.map((trailer, index) => index < 4 && (
                        <Trailer animationDelay={index === 0 ? 0 : index * 200} key={index} videoUrl={trailer.video_url} setTrailerUrl={setTrailerUrl} setIsModalOpen={setIsModalOpen}  />
                    ))}
                </div>
                    ...
                    { trailers.length > 0 && 
                        <Carousel maxWidth="100%" title="Últimos trailers">
                            {trailers.map((result, index) => (
                                <Trailer animationDelay={0} videoUrl={result.video_url} setIsModalOpen={setIsModalOpen} setTrailerUrl={setTrailerUrl} key={index}/>
                            ))}
                        </Carousel>
                    }
                </div>
